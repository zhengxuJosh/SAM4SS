...
class Sam(nn.Module):
    ...
    def forward_test(
        ...
        input_images = torch.stack([self.preprocess(x.squeeze(0)) for x in batched_input], dim=0)
        ...
        for image_record, curr_embedding in zip(batched_input, image_embeddings):
            ...
            sparse_embeddings, dense_embeddings = self.prompt_encoder(
                points=points,
                boxes = None, # boxes=image_record.get("boxes", None), 
                masks = None, # masks=image_record.get("mask_inputs", None),
            )
            low_res_masks, iou_predictions = self.mask_decoder(
            ...
            masks = self.postprocess_masks(
                low_res_masks,
                input_size=[1024,1024],# image_record["image"].shape[-2:],
                original_size=1024,# image_record["original_size"],
            )
